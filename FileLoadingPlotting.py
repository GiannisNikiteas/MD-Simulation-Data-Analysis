...
def radial_dist_func(power, par_a):
    ...
    plt.figure(3)
    iso = np.sqrt(1 - par_a)
    plt.plot(x, Hist, 'o-', markersize=4, label=name, color=color_sequence2[c])
    plt.xlabel(r"$r$", fontsize=16)
    plt.ylabel(r"$g(r)$", fontsize=16)
    plt.plot([0, x[-2]], [1, 1], '--', color='black', linewidth=0.5)
    plt.plot([iso, iso], [0, max_scaling + 0.1], '--', color='red')
    plt.xlim(xmin=0, xmax=x[-2])
...
def vel_autocor_func(power, par_a):
    ...
    plt.legend(loc="upper right", ncol=1, borderpad=0.1, labelspacing=0.01, columnspacing=0.01, fancybox=True,
               fontsize=16)
    p += 1
